...
import excel .\data_svar_rob.xlsx,/*
...
label var eui "Energy Uncertainty Index"
label var eui_w "Energy Uncertainty Index Weighted"
...
quietly lpirf lpro ldem lrpo eui_w, step(41) lags(1/24) ///
  exog(L(0/24).e1) vce(robust)
irf create lpmodel 

// quietly var lpro ldem lrpo eui, lags(1/24)            ///
//   exog(L(0/24).e1)
// irf create varmodel, step(24)

irf graph dm, impulse(e1) response(lrpo)   ///
  irf(lpmodel) level(95) name(G2e, replace) ///
    xline(0 10 20 30 40 50, lcolor(blue)) yline(-.05 0 .05 .1, lcolor(blue))

graph export "G2_e.svg", replace
graph export "G2_e.png", as(png) width(4000) replace
graph export "G2_e.pdf", as(pdf) replace
...
// quietly var lpro ldem lrpo eui_w, lags(1/24)            ///
//   exog(L(0/24).e_1)
// irf create varmodel1, step(30)

irf graph dm, impulse(e_1) response(lrpo)   ///
  irf(lpmodel1) level(95) name(G4e, replace) ///
    xline(0 10 20 30 40 50, lcolor(blue)) ///
	yline(-0.1 -.05 0 .05 .1, lcolor(blue))

graph export "G4e.svg", replace
graph export "G4e.png", as(png) width(4000) replace
graph export "G4e.pdf", as(pdf) replace

irf set comparemodels1.irf, replace
quietly lpirf lpro ldem lrpo eui_w, step(41) lags(1/24) ///
  exog(L(0/24).gprcn) vce(robust)
irf create lpmodel1
...
irf graph dm, impulse(gprcn) response(lrpo)   ///
  irf(lpmodel1) level(95) name(G4eb, replace) ///
    xline(0 10 20 30 40 50, lcolor(gray)) ///
	yline(-.5 0 .5 1, lcolor(black))

graph export "G4eb.svg", replace
graph export "G4eb.png", as(png) width(4000) replace
graph export "G4eb.pdf", as(pdf) replace
