# This code reproduces the result in Figure A.3 of the supplemental online appendix
...
for (r in 1:N) { # estimate local projections for each GPR index
  ...
  for (h in 1:(H+1)) {
    ...
    regr_temp_robust <- coeftest(regr_temp, vcov. = vcovHC(regr_temp, type = "HC1"))
    # type HC1 is the White estimator with the correction for the degrees of freedom
