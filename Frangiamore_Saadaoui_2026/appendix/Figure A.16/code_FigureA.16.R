# This code reproduces the result in Figure A.16 of the supplemental online appendix
...
# take local GPR indices
GPR_local = cbind(data$Germany,data$France,data$Italy,data$Spain,data$Belgium,data$Ireland,data$Austria)
...
# take country-specific SovCISS indices to be added as controls
SovCISS_c <- cbind(data$SovCISS_Germany,data$SovCISS_France,data$SovCISS_Italy,
                   data$SovCISS_Spain,data$SovCISS_Belgium,data$SovCISS_Ireland,data$SovCISS_Austria)
...
for (r in 1:N) { # estimate local projections for each GPR index
  ...
  SovCISS_local = SovCISS_c[,r] # take country-specific SovCISS
  
  # prepare variables (first observation is removed because of the computation of 
  # industrial production growth)
  data_temp = cbind(SovCISS[-1],GPR[-1],IP_gr,int_rate[-1],SovCISS_local[-1]) 
...
# Plot IRFs to country-level GPR of biggest Euro Area countries (Germany, France, Italy, Spain)
par(mfrow = c(3,3),mar=c(3.5, 3, 1.5, 1))
for (r in 1:N) {
