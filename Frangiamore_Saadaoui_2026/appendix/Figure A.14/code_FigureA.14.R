# This code reproduces the result in Figure A.14 of the supplemental online appendix
...
# take stock prices and compute returns to be added as additional control
SP = data$SHIX_EA
return = diff(log(SP))*100
...
for (r in 1:N) { # estimate local projections for each GPR index
  ...
  data_temp = cbind(SovCISS[-1],GPR[-1],IP_gr,int_rate[-1],return) 
