# This code reproduces the result in Figure A.11 of the supplemental online appendix
...
# take price index and compute inflation to be added as additional control
HICP = data$HICPOV_EA  
infl = diff(log(HICP))*100
...
for (r in 1:N) { # estimate local projections for each GPR index
  ...
  data_temp = cbind(SovCISS[-1],GPR[-1],IP_gr,int_rate[-1],infl) 
